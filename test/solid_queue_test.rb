...
class SolidQueueTest < ActiveSupport::TestCase
  ...
  test "schedules recurring tasks" do
    SolidQueue.schedule_recurring_task("test 1", command: "puts 1", schedule: "every hour")
    SolidQueue.schedule_recurring_task("test 2", command: "puts 2", schedule: "every minute", static: true)
  ...
  test "schedules recurring tasks with class and args (same keys as YAML config)" do
    SolidQueue.schedule_recurring_task("test 3", class: "AddToBufferJob", args: [ 42 ], schedule: "every hour")
  ...
  test "unschedules recurring tasks" do
    ...
    SolidQueue.unschedule_recurring_task(dynamic_task.key)

    assert_raises(ActiveRecord::RecordNotFound) do
      SolidQueue.unschedule_recurring_task(static_task.key)
    end
  ...
  test "schedule_recurring_task with duplicate key raises error" do
    SolidQueue.schedule_recurring_task("duplicate_test", command: "puts 1", schedule: "every hour")

    assert_raises(ActiveRecord::RecordNotUnique) do
      SolidQueue.schedule_recurring_task("duplicate_test", command: "puts 2", schedule: "every minute")
    end
  end

  test "unschedule_recurring_task with nonexistent key raises RecordNotFound" do
    assert_raises(ActiveRecord::RecordNotFound) do
      SolidQueue.unschedule_recurring_task("nonexistent_key")
    end
  end
end
